	...
	public function performance_trend(Request $request) 
	{
		...
		if ($frequency->frequency_month_value > 1) {
				...
				foreach ($action_plans as $a) {
					$action_item = [
						[
						   ...
						   "x" => '$dataset.0.set.'.$a->month_no.'.x-25',
						   "y" => '$dataset.0.set.'.$a->month_no.'.starty-15',
						   "tox" => '$dataset.0.set.'.$a->month_no.'.x+25',
						   "toy" => '$dataset.0.set.'.$a->month_no.'.starty-40'
						],
						[
						   ...
						   "y" => '$dataset.0.set.'.$a->month_no.'.starty-16',
						   "radius" => "11",
						],		
						[
						   ...
						   "text" => "SIP",
						   "x" => '$dataset.0.set.'.$a->month_no.'.x-',
						   "y" => '$dataset.0.set.'.$a->month_no.'.starty - 27'
						]
					];
			...
			if ($request->appraisal_type_id == 2) {
				...
				$period = AppraisalPeriod::find($request->period_id);
				$org_list = DB::select("
					select distinct e.org_id, d.emp_id, i.emp_name org_name, d.item_result_id, f.result_threshold_group_id, g.item_name, h.perspective_name
					from monthly_appraisal_item_result a
				...
				$period = AppraisalPeriod::find($request->period_id);
				$org_list = DB::select("
					select distinct e.org_id, d.emp_id, e.org_name, d.item_result_id, f.result_threshold_group_id, g.item_name, h.perspective_name
					from monthly_appraisal_item_result a
					left outer join appraisal_period b
					on a.period_id = b.period_id
					left outer join appraisal_frequency c
